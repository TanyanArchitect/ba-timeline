  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50 bg-slate-900/95 backdrop-blur-sm border-b border-cyan-900 shadow-xl p-4">
      <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-center gap-4">
        <span className="text-cyan-400 font-bold mr-2 uppercase text-sm tracking-widest">
          Filters:
        </span>
        ...
        {FILTER_OPTIONS.map((option) => {
          ...
          return (
            <label 
              key={option.value} 
              className={`
                cursor-pointer flex items-center space-x-2 px-3 py-1 rounded-full border transition-all select-none
                ${isActive 
