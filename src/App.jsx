...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans overflow-x-hidden pt-24 pb-0 relative">
      <FilterBar selectedFilters={selectedFilters} onToggleFilter={handleToggleFilter} />
        ...
        {filteredRows.map((row, rowIndex) => {
          ...
          return (
            <div 
              key={rowIndex} 
              className={`
                relative flex justify-between mb-32
                items-start
                flex-col gap-10 md:flex-row md:gap-0
                ${isReverse ? 'md:flex-row-reverse' : ''}
              `}
            >
              
              <div className="absolute top-4 left-0 w-full h-1 bg-cyan-900 -z-10 hidden md:block"></div>

              {!isLastRow && (
                <>
                  {!isReverse && (
                     <div className="hidden md:block absolute right-0 top-4 w-24 h-[calc(100%+8rem)] border-r-4 border-t-4 border-b-4 border-cyan-900 rounded-r-[3rem] -mr-[3rem] -z-10"></div>
                  )}
                  {isReverse && (
                     <div className="hidden md:block absolute left-0 top-4 w-24 h-[calc(100%+8rem)] border-l-4 border-t-4 border-b-4 border-cyan-900 rounded-l-[3rem] -ml-[3rem] -z-10"></div>
                  )}
                </>
              )}
              ...
              {row.map((story) => (
                ...
                >
                  ...
                  <div className="mt-3 text-center opacity-80 group-hover:opacity-100 transition-opacity w-full px-2">
                    
                    <span className="text-[10px] uppercase tracking-wider text-cyan-300 bg-cyan-900/40 px-2 py-1 rounded inline-block mb-1">
                      {story.type}
                    </span>

                    {story.sub_title && (
                      <p className="text-xs font-bold text-cyan-400 uppercase tracking-wide mt-2 line-clamp-1">
                        {story.sub_title}
                      </p>
                    )}

                    <h3 className="text-sm font-bold text-slate-200 group-hover:text-yellow-300 mt-1 leading-tight">
                      {story.display_title}
                    </h3>
